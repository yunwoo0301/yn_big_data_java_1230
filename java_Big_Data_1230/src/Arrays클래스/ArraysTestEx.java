...
// sort() : 기본적인 오름차순, 그리고 매개변수를 이용한 내림차순, 그리고 오버라이딩 통한 직접 구현
...
public class ArraysTestEx {
    public static void main(String[] args) {
        ...
        int num = sc.nextInt();
        Integer[] arr = new Integer[num]; // 해당 표현 주의하기!
        System.out.println("찾을 수 입력 : ");
        int key = sc.nextInt();
        for(int i = 0; i < arr.length; i++) {
            arr[i] = (int)(Math.random() * num) + 1; // 배열의 개수 범위네에서의 값으로 난수 발생
        }
        ...
        // 결과값으로 해당값이 있는 배열의 인덱스가 변환함. 찾는 값이 없으면 음수가 반환됨
