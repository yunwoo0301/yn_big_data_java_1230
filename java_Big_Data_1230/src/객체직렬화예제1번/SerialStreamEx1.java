...
// 직렬화란? 프로그램에서 사용되는 객체를 파일이나 네트워크로 전송하기 위해 연속적인 바이트 형태로 변경하는 것
// 직렬화된 바이트를 객체로 복원하는 것을 역직렬화라고 함.
public class SerialStreamEx1 {
    public static void main(String[] args) throws IOException, ClassNotFoundException {
        writeList();
        List<Board> list = readList();
        SimpleDateFormat sdf = new SimpleDateFormat("yyyy-MM-dd"); // 데이터값을 원하는 형식에 맞춰서 출력해야하기 때문
        for(Board e : list) {
    ...
    static void writeList() throws IOException { // 객체애 대한  직렬화
        ...
        oos.flush(); // write 이후에 반드시 flush
        oos.close();
    }
    static List<Board> readList() throws IOException, ClassNotFoundException { // 백엔드 구현 시 많이 사용함(중요함)
