...
public class AirCon {
    private boolean isPower; //2개 값만 있으므로 boolean 설정, 전원 ON/OFF
    ...
    public AirCon() { // 에어컨 생성자
        final int[] monthTemp = {-5, 3, 8, 14, 22, 27, 31, 30, 22, 18, 7, 2};
        Calendar now = Calendar.getInstance(); // Calendar : 추상 클래스
    ...
    public void setAirConState() {
        ...
        windStep = sc.nextInt();
        if(currTemp > setTemp) { // 현재 온도가 더운 상태
            System.out.println("Cooler 가 동작 합니다.");
            isCooler = true;
            isHeater = false;
        } else if(currTemp < setTemp) { // 현재 온도가 추운 상태
